...
async def _ensure_user_exists(user_id: int) -> None:
    async with httpx.AsyncClient(timeout=5.0) as client:
        try:
            response = await client.get(f"{AUTH_URL}/internal/users/{user_id}/msgpack")
...
async def _notify_about_order(order: models.Order) -> None:
    ...
    async with httpx.AsyncClient(timeout=5.0) as client:
        try:
            await client.post(f"{NOTIFICATION_URL}/internal/order-created/msgpack", json=payload)
            await client.post(
                f"{TRACKING_URL}/tracking/internal/order-created/msgpack",
